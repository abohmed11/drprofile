...
target = """                <h3 className="font-extrabold text-sm text-neutral-900 truncate">{formData.name || 'دكتور بروفايل'}</h3>
...
replacement = """                <h3 className="font-extrabold text-sm text-neutral-900 truncate">{formData.name || 'دكتور بروفايل'}</h3>
                <span className="text-[9px] text-[#10244A] font-extrabold uppercase mt-0.5">
                  {loggedSecretary ? `سكرتارية: ${loggedSecretary.name}` : (formData.jobTitle || 'لوحة تحكم الطبيب')}
                </span>
              </div>
